<template>
    <side-slide :is-open="isOpen" :panel-title="panelTitle" @close="isOpen = false">
        <div v-if="panelTitle === 'Characters'">
            <div class="container">
                <div class="mb-4">
                    <form-input v-model="newCharacter.name" label="Name" type="text" name="name" required="true"
                        :value="newCharacter.name"></form-input>
                    <form-input v-model="newCharacter.picture_url" label="Img URL" type="text" name="URL"
                        required="true" :value="newCharacter.picture_url"></form-input>
                    <button @click="addCharacter()"
                        class="mt-2 bg-indigo-500 text-white font-medium py-2 px-4 rounded-md hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-700">
                        {{ isEditing ? 'Save Changes' : 'Add' }}
                    </button>
                </div>
            </div>
            <div v-if="game.characters.length > 0"
                class="container mx-auto px-4 border-dashed border-2 border-sky-500 min-h-10 rounded-md flex flex-row flex-wrap justify-around">
                <div v-for="g in game.characters"
                    class="w-2/5 rounded overflow-hidden shadow-lg bg-gray-100 my-2 basis-1/3">
        ...
        <div v-else>
            <div class="container">
                <div class="mb-4">
                    <form-input v-model="newSoundtrack.title" label="Title" type="text" name="title" required="true"
                        :value="newSoundtrack.title"></form-input>
                    <form-input v-model="newSoundtrack.video_url" label="Video URL" type="text" name="URL"
                        required="true" :value="newSoundtrack.video_url"></form-input>
                    <button @click="addSoundtrack()"
                        class="mt-2 bg-indigo-500 text-white font-medium py-2 px-4 rounded-md hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-700">
                        Add
                    </button>
                </div>
            </div>
            <div v-if="game.soundtracks.length > 0"
                class="container mx-auto px-4 border-dashed border-2 border-sky-500 min-h-10 rounded-md flex flex-row flex-wrap justify-around">
                <div v-for="ost in game.soundtracks"
                    class="w-2/5 rounded overflow-hidden shadow-lg bg-gray-100 my-2 basis-1/3">
                    <div class="aspect-w-4 aspect-h-4">
                        <iframe width="100%" height="100%" :src="ost.video_url" title="YouTube video player"
                            frameborder="0"
    ...
    <div v-if="gameData">
        <form @submit.prevent="submitForm" class="container mx-auto mt-8 mb-8 p-6 bg-gray-100 rounded-lg shadow-md">
            <div class="mb-4">
                <form-input v-model="game.title" label="Game Title" type="text" name="title" required="true"
                    :value="game.title"></form-input>
            </div>

            <div class="mb-4">
                <label for="description" class="block text-gray-700 font-medium mb-2">Description:</label>
                <textarea v-model="game.description" id="description" required="true"
            ...
            <div class="mb-4">
                <select-input name="genre" v-model="game.genre" :items="genres" required="required" label="Genre"
                    :value="game.genre"></select-input>
            </div>
            ...
            <div class="mb-4">
                ...
                <p v-if="game.ratings.main != 1 && game.ratings.main" class="block text-gray-700 font-medium mb-2">main:
                </p>
                <div class="columns-4">
                    <SelectInput name="story" v-model="game.ratings.story" :items="scores" required="required"
                        label="Story" :value="game.ratings.story"></SelectInput>
                    <SelectInput name="ost" v-model="game.ratings.ost" :items="scores" required="required" label="Ost"
                        :value="game.ratings.ost"></SelectInput>
                    <SelectInput name="art" v-model="game.ratings.art" :items="scores" required="required" label="Art"
                        :value="game.ratings.art"></SelectInput>
                    <SelectInput name="gameplay" v-model="game.ratings.gameplay" :items="scores" required="required"
                        label="Gameplay" :value="game.ratings.gameplay"></SelectInput>
                </div>
            </div>
            ...
            <div class="mb-4 columns-2">
                <div>
                    ...
                    <flat-pickr v-model="game.dateStart" :config="config"
                        class="border border-gray-300 rounded-md px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        placeholder="Select date" name="date" />
                </div>
                <div>
                    <label class="block text-gray-700 font-medium mb-2">End Date</label>
                    <flat-pickr v-model="game.dateEnd" :config="config"
                        class="border border-gray-300 rounded-md px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        placeholder="Select date" name="date" />
                </div>
            </div>
            ...
            <div class="mb-4 columns-2">
                <div>
                    <button @click="openSideSlide('Characters')" type="button"
                        class="bg-indigo-700 text-white font-medium py-2 px-4 rounded-md hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-900 flex items-center">
                ...
                <div>
                    <button @click="openSideSlide('Soundtracks')" type="button"
                        class="bg-indigo-700 text-white font-medium py-2 px-4 rounded-md hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-900 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
            ...
            <div class="mb-4 columns-2">
                <form-input v-model="game.trailer_url" label="Game Trailer" type="text" name="trailer" required="true"
                    :value="game.trailer_url"></form-input>
                <form-input v-model="game.wiki_url" label="Game Wiki" type="text" name="wiki" required="true"
                    :value="game.wiki_url"></form-input>
            </div>

            <button type="submit"
                class="bg-indigo-500 text-white font-medium py-2 px-4 rounded-md hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-700">
                Update
            </button>
            <div v-if="isLoading">
...
async function fetchGameData() {
    try {
        ...
        const wallpaper = gameData.value.wallpaper;
        imageToEdit.value = wallpaper && wallpaper.startsWith('http')
            ? wallpaper
            : `${buildApiUrl('')}${wallpaper}`;
    } catch (error) {
